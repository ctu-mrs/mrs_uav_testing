...
namespace mrs_testing
{
  ...
  void ControlTest::onInit() {
    ...
    param_loader.load_param("min_xy", min_xy_);
  ...
  void ControlTest::mainTimer([[maybe_unused]] const ros::TimerEvent &event) {
    ...
    switch (current_state) {
      ...
      case LAND_HOME_STATE: {
        ...
        if (tracker_status.tracker.compare("mrs_uav_manager/NullTracker") == 0 && dist2d(home_x, odometry_x, home_y, odometry_y) < 1.0) {
      ...
      case LAND_STATE: {
        ...
        if (tracker_status.tracker.compare("mrs_uav_manager/NullTracker") == 0 && dist2d(des_x, odometry_x, des_y, odometry_y) < 1.0) {
          changeState(ControlState_t(int(current_state) + 1));
        }
      } break;
  ...
  bool ControlTest::inDesiredState(void) {

    {
      ...
      // TODO: uncoment
      if (dist3d(odometry_x, des_x, odometry_y, des_y, odometry_z, des_z) < 0.15 &&
          (headless || fabs(sanitizeYaw(odometry_yaw) - sanitizeYaw(des_yaw)) < 0.15)) {
