...
~/.i3/detacher.sh "~/.scripts/set_ros_master_uri.sh $UAV_NAME"
...
# 'name' 'command'
input=(
  'RVIZ' "waitForRos; roscd mrs_testing; ./scripts/change_uav.sh $UAV_NAME; rosrun rviz rviz -d rviz/odometry.rviz
  "
  ...
  "
  'Status' "waitForOdometry; roslaunch mrs_status status.launch
  "
)
